...
//print_r(getimagesize("http://fotos.centrina.com.br/1390/11877531.jpg"));exit;

///print_r($_SERVER);exit;
//		echo "<img src='".site::site_thumb()."r_1457349.jpg' />" ;exit;
	...
	{				
		...
		$objs = busca::busca($filtros,$_POST['ord'],$_POST['pag'],10,"lista_imoveis",null,true,true); //busca
    ...
    public function busca() //busca por qualquer coisa
    {		
		//print_r($_POST);exit;
        $busca = $this->input->post('cod_jb');
        ...
        if($busca){
            $row = ORM::factory('imovel')->where('cod_jb',$busca)->find();
			
            if( $row->loaded ){	//buscou pelo código
            ...
            else //se não achou vamos buscar por qualquer outra coisa
            {
            	$filtros = array();
            	$db = new Database();
            	$query = $db->query("
            		SELECT cod_jb,
	            		COALESCE(
						    CASE WHEN tipo LIKE '%".$busca."%' THEN 'tipo' END,
						    CASE WHEN finalidade LIKE '%".$busca."%' THEN 'finalidade' END,
						    CASE WHEN cidade LIKE '%".$busca."%' THEN 'cidade' END,
						    CASE WHEN bairro LIKE '%".$busca."%' THEN 'bairro' END,
						    CASE WHEN uf LIKE '%".$busca."%' THEN 'uf' END
						    ) as matched
						FROM imoveis 
						WHERE tipo LIKE '%".$busca."%' OR finalidade LIKE '%".$busca."%' OR cidade LIKE '%".$busca."%' OR bairro LIKE '%".$busca."%' OR uf LIKE '%".$busca."%'     
					    group by matched
            		");
            	...
            	{
            		switch ($i->matched) {
            			//case 'tipo': $filtros[] = $i->matched."=".$busca;	break;
            			
            			default:$filtros[] = $i->matched."=".$busca;break;
            		}
		        	
            	}
				$filtros[]	= "pret=locacao";
		       
            	//print_r($filtros);
            	if( count($filtros > 0) ) //se foi encontrado alguma coisa
            	{    
            		$url = "imoveis_a-venda";
            		foreach($filtros as $f)
            		{	echo $url."<br>";
            			$url = tools::trata_url_imovel($url,$f,true,true);
            			echo $url."<br>";
            		}
            			exit;            		
            		 url::redirect ($url);
				}
				else//se nao achou nada
				{
					 // monta lista de imoveis
		            $this->template->layout = new View('nao_encontrado');
					$this->template->body_id = "busca";
		            $this->template->layout->cod_jb = new View('pesquisa_cod_jb');
		            $this->template->layout->codigo = $cod_jb;
		            $this->template->layout->cod_jb->pret = $pret;            			
					$this->template->layout->pret = $pret;            			
					$this->template->layout->destaques = busca::busca($filtros, 'recentes', 1, 20);       
				}
            }                           
            ...
            if ( $this->input->post('pret_busca') )$url .= "&pret=" . $this->input->post('pret_busca');
            if ( $this->input->post('tipo') )$url .= "&tipo=" . $this->input->post('tipo');
            if ( $this->input->post('finalidade') ) $url .= "&finalidade=" . $this->input->post('finalidade');
            $url .= "&cidade=" . $cidade; 
            if ( $this->input->post('bairro') ) $url .= "&bairro=" . $this->input->post('bairro');	
	
			$faixa_valor = $this->input->post('faixa_valor_min_'.$pret)."-a-".$this->input->post('faixa_valor_max_'.$pret);			
			if($faixa_valor == "qualquer-valor-a-qualquer-valor")
				$faixa_valor = "";
				
            $url .= "&faixa_valor=" . $faixa_valor;		
            if ( $this->input->post('banheiros') ) $url .= "&banheiros=" . $this->input->post('banheiros');
            if ( $this->input->post('dorm') ) $url .= "&dormitorios=" . $this->input->post('dorm');
            if ( $this->input->post('garagens') ) $url .= "&garagens=" . $this->input->post('garagens');
			
			//echo $url;exit;
				
            url::redirect ( tools::trata_url_imovel( $url ) );
		}	        
