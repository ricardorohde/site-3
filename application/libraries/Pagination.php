...
class Pagination_Core {
	...
	public function initialize($config = array())
	{
		...
		if ($this->query_string !== '')
		{
			...
			// Insert {page} placeholder
			$_GET[$this->query_string] = '';

			// Create full URL
			$base_url = ($this->base_url === '') ? Router::$current_uri : $this->base_url;
			$this->minha_pagina = url::site($base_url).'?'.str_replace('%7Bpage%7D', '{page}', http_build_query($_GET));
		...
		{
			// Use current URI if no base_url set
			$this->minha_pagina = ($this->base_url === '') ? Router::$segments : explode('/', trim($this->base_url, '/'));
			...
			if (is_string($this->uri_segment))
			{
				...
				{
					// If uri 'label' is not found, auto add it to base_url
					$this->minha_pagina[] = $this->uri_segment;
					$this->uri_segment = count($this->url) + 1;
				}
				else
				{
			...
			// Insert {page} placeholder
			$this->minha_pagina[$this->uri_segment - 1] = '{page}';

			// Create full URL
			$this->minha_pagina = url::site(implode('/', $this->minha_pagina)).Router::$query_string;
