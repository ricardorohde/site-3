...
function pega_url()
{
	var path = window.location.pathname.split("/")[2]; //index da url com os filtros
	path = (path.substring(0,(path.length-5)));
	return path;
}

function pesquisa(a,pagina,ordem)
{		
	...
	$.ajax({
	  ...
	  data: ({dado:a,pret:pret,filtros:path,ord:ordem,pag:pagina}),	
	  dataType:"json",	
	  beforeSend: function(){	
	  ...
	  success: function(ret) {	
		...
		{  
			...
			if(paginador["previous_page"]!==false) //mostrar só se possuir pagina anterior/próxima
				$(".previous2").html("<a href='"+window.location+"#' onclick=pesquisa('nao-entrar',"+paginador["previous_page"]+",'"+ret["ord_atual"]+"')></a>");	
				
			if(paginador["next_page"]!==false)
			$(".next2").html("<a href='"+window.location+"#' onclick=pesquisa('nao-entrar',"+paginador["next_page"]+",'"+ret["ord_atual"]+"')></a>");	
...
$(document).ready(function() {
	...
	pesquisa("nao-entrar",<?php echo ( isset($_GET['pagina']) )?($_GET['pagina']):("1"); ?>,'<?php echo ( isset($_GET['ord']) )?($_GET['ord']):("valor-desc"); ?>'); 
