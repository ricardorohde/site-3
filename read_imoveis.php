 ...
 /* echo '<pre>';
  ...
  if ( sizeof( $lista ) > 0 ){
    ...
    foreach( $lista as $row ){
      ...
      // ***** INCLUI OU ATUALIZA O IMOVEL NO PRM *****
      // *** PRM ****
      /*
      $prm_api->criarOuAtualizarProduto(
          array(
              'referencia' => $row['cod_jb'],
              'nome_curto' => $row['cod_jb'],
              'titulo' => sprintf('%s - %s - %s - %s - %s - %s', $row['cod_jb'], $row['tipo'], $row['finalidade'], $row['bairro'], $row['cidade'], $row['uf']),
              'preco_unitario' => $row['valvenda'] > 0 ? $row['valvenda'] : $row['vallocacao'],
    ...
    $resultado = mysql_query($query,$conexao);
//print_r($lista_imagens);exit;
    foreach( $lista_imagens as $row ){
	   // print_r($row);
      
      if($row["minia"]==1){
         $image = new SimpleImage(); 
         $image->load( $row['arqfoto'] ); 
